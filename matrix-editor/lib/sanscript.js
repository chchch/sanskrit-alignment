...
export class Sanscript {
     ...
     */
        devanagari: {
            ...
            // Dummy consonant. This is used in ITRANS to prevent certain types
     ...
     */
    isRomanScheme (name) {
        return this.romanSchemes.hasOwnProperty(name);
    };

    /**
     * Add a Brahmic scheme to this.
     *
     ...
     *                described above.
     */
    addBrahmicScheme (name, scheme) {
        this.schemes[name] = scheme;
    };

    /**
     * Add a roman scheme to this.
     *
     * See the comments on this.addBrahmicScheme. The "vowel_marks" field
     * can be omitted.
     *
     * @param name    the scheme name
     * @param scheme  the scheme data itself
     */
    addRomanScheme(name, scheme) {
    ...
    constructor () {
        ...
        const kolkata = this.schemes.kolkata = this.cheapCopy(this.schemes.iast),
          schemeNames = 'iast itrans hk kolkata slp1 velthuis wx'.split(' ');
        kolkata.vowels = 'a ā i ī u ū ṛ ṝ ḷ ḹ e ē ai o ō au'.split(' ');

        // These schemes already belong to this.schemes. But by adding
        // them again with `addRomanScheme`, we automatically build up
        // `this.romanSchemes` and define a `vowel_marks` field for each one.
        ...
        const itrans_dravidian = this.cheapCopy(this.schemes.itrans);
     ...
     */
    makeMap (from, to, /*options*/) {
        const alternates = this.allAlternates[from] || {},
          consonants = {},
     ...
     */
    transliterateRoman (data, map, options) {
        const buf = [],
     ...
     * @return         the finished string
     */
    transliterateBrahmic (data, map, /*options*/) {
        const buf = [],
          consonants = map.consonants,
          letters = map.letters,
          marks = map.marks,
     ...
     * @return         the finished string
     */
    t (data, from, to, options) {
        options = options || {};
        const cachedOptions = this.cache.options || {},
          defaults = this.defaults;
        let hasPriorState = (this.cache.from === from && this.cache.to === to),
          map;
