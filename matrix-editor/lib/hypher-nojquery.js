...
const hyphenate = function(node,language) {
        if(window['Hypher']['languages'][language]) {
            var i = 0, len = node.childNodes.length;
                for (; i < len; i += 1) {
                    if (node.childNodes[i].nodeType === 3) {
                        node.childNodes[i].nodeValue = window['Hypher']['languages'][language].hyphenateText(node.childNodes[i].nodeValue);
                    }
                    else if(node.childNodes[i].nodeType === 1)
                        hyphenate(node.childNodes[i],language);
                }
        }
};
